...
import { getReleaseRequests, addReleaseRequestToDeliveryTable, addReleaseRequestToDocsTable } from "~/backend/release";
...
export default function Requests() {
    ...
    async function handleAddRequestToDocsTable(id: string) {
        setModalIsOpened(true);
        const result = await addReleaseRequestToDocsTable(id);
        console.log(result)
        if (result !== 200) {
            setModalIsOpened(false);
            alert(result.error)
        } else {
            alert('Релиз добавлен в таблицу документов')
        }   
        setModalIsOpened(false);
    }
                    ...
                    requests.map((release, index) => {
                        ...
                        return (
                            <div key={index} className="release-container" style={{ marginBottom: "1.7vh" }}>
                                ...
                                <svg onClick={() => handleAddRequestToDocsTable(release.id)} className={index === 0 ? "track-controls-first" : "track-controls"} style={{ marginLeft: '1vw', marginRight: '1vw', cursor: 'pointer' }} width="29" height="29" viewBox="0 0 29 29" fill="none" xmlns="http://www.w3.org/2000/svg">
