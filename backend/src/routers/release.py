...
async def add_to_docs(id: str):
    ...
    if release_type == 'clip':
        ...
        for i, track in enumerate(tracks):
            ...
            tracks_enumed_music_authors.append(f"{i + 1}. {', '.join(track_music_authors_names) if len(track_music_authors_names) else '-'}") 
            tracks_enumed_lyricists.append(f"{i + 1}. {', '.join(track_lyricists_names) if len(track_lyricists_names) else '-'}")
            tracks_enumed_phonogram_producers.append(f"{i + 1}. {', '.join(track_phonogram_producers_names) if len(track_phonogram_producers_names) else '-'}")
